...
namespace Game1
{
	...
	{
		public Enemy2 (GraphicsContext g, Collision c, Vector3 pos):base (g, c, new Texture2D("/Application/Assets/player.png",false), pos)
		{
			Duration = 50;
			Size = 40;
			Frames = 8;
			MovementSpeed = 5;
			s.SetColor(new Vector4(0f, 1f, 0f, 1f));
		}
		...
		{
			Time += elapsed;
			Vector3 randPos = new Vector3(rand.Next(30, 930), rand.Next (30, 514),0);
			Vector3 tempPos = this.s.Position;
			float deltaX = tempPos.X - randPos.X;
			float deltaY = tempPos.Y - randPos.Y;
